...
sgtreec_module = Extension('sgtreec',
        ...
        include_dirs=['lib/'],
        extra_compile_args=[ '-pthread', '-std=c++14'], #, '-DPRINTVER'], #, '-D_FLOAT64_VER_'],
        extra_link_args=[ '-pthread', '-std=c++14'] #,'-DPRINTVER'] #, '-D_FLOAT64_VER_'], #'-D_FLOAT64_VER_', '-DPRINTVER'
)
...
scc_module = Extension('sccc',
        ...
        extra_link_args=[ '-pthread', '-std=c++14'] #'-DPRINTVER']
)
...
setuptools.setup(
    ...
    ext_modules = [ scc_module, covertreec_module, sgtreec_module],
    python_requires=">=3.6",
)
